...
fn source(
    reponses: Vec<EchangeSimule>,
) -> (
    EnableBankingBankDataSource<Arc<TransportSimule>>,
    Arc<TransportSimule>,
) {
...
async fn initier_consentement_renvoie_url_de_redirection_et_consent_pending() {
    let reponse =
        r#"{"url":"https://banque.example/authorize?id=abc","authorization_id":"auth-123"}"#;
    ...
    assert!(
        initie
            .url_autorisation
            .starts_with("https://banque.example/authorize?id=abc")
    );
    assert!(
        initie
            .url_autorisation
...
async fn solde_mappe_booked_et_available_en_centimes() {
    ...
    assert_eq!(
        transport.requetes()[0].chemin,
        "/accounts/acc-eb-001/balances"
    );
}
